...
#define RD "C:/Users/xxyofu/Desktop/test.ppm"
#define WD "C:/Users/xxyofu/Desktop/test2.ppm"
...
public:
    ...
    void DrawFunk(  Point zero, int l, RGB color)
    {
        for (int i = 1; i <= l; i += 1) 
        {
            ...
            DrawLine({zero.x+i,zero.y-int(log(double(i)/100))}, {zero.x+i+1,zero.y-int(log(double(i+1)/100))}, color);
        }
    }
