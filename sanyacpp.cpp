...
public:
    ...
    void DrawFunk(  Point zero, int l, RGB color)
    {
        for (int i = 1; i <= l; i += 1) 
        {
            ...
            DrawLine({zero.x+i,zero.y-int(sqrt(double(i)))}, {zero.x+i+1,zero.y-int(sqrt(double(i+1)))}, color);
        }
    }
...
int main(int argc, char *argv[])
{
    ...
    RGB color = { 0,0,0 };
    RGB bl = { 0,0,255 };
    canvas.CopyPart(img, {canvas.width/2, canvas.height-40},{img.width/2+40, img.height-40},40);
